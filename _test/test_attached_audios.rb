...
class TestVkMusic < MiniTest::Test
  ...
  def test_simple
    re = CLIENT.attached_audios(url: 'https://vk.com/wall-184089233_6').size
    assert_equal(1, re, 'This post got only 1 attached audio')
  end

  def test_empty
    re = CLIENT.attached_audios(url: 'https://vk.com/wall-184089233_2').size
  ...
  def test_single_audio_with_options
    re = CLIENT.attached_audios(owner_id: -184_089_233, post_id: 6).size
    assert_equal(1, re, 'This post got only 1 attached audio')
  end
  ...
  def test_url_to_reply
    re = CLIENT.attached_audios(url: 'https://m.vk.com/wall-4790861_10108').size
    assert_equal(1, re, 'Although this link redirects to comment, audios from post must be parsed')
  end

  def test_playlist
    re = CLIENT.attached_audios(url: 'vk.com/wall-184089233_4').size
    assert_equal(0, re, 'This post got no attached audios')
  end

  def test_comments_with_audios
    re = CLIENT.attached_audios(url: 'https://m.vk.com/wall-39786657_189247').size
    assert_equal(1, re, 'This post got 1 attached audio')
  end

  def test_repost_with_no_audios
    re = CLIENT.attached_audios(url: 'https://vk.com/wall-184936953_1').size
    assert_equal(0, re, 'This post got no attached audios')
  end

  def test_repost_with_audios
    re = CLIENT.attached_audios(url: 'https://vk.com/wall-184936953_2').size
    assert_equal(1, re, 'This post got 1 attached audio')
  end

  def test_repost_with_playlist
    re = CLIENT.attached_audios(url: 'https://vk.com/wall-184936953_3').size
    assert_equal(0, re, 'This post got no attached audios')
  end

  def test_bad_url
    assert_raises(VkMusic::ParseError) do
      CLIENT.attached_audios(url: 'ae')
    end
  end

  def test_nonexistent_post
    assert_raises(VkMusic::ParseError) do
      CLIENT.attached_audios(url: 'https://m.vk.com/wall-4790861_1052600000')
    end
  end
end

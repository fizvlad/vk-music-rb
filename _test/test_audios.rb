...
class TestVkMusic < MiniTest::Test
  def test_user
    pl = CLIENT.audios(owner_id: 8_024_985)
  ...
  def test_incorrect_id
    assert_raises(VkMusic::ParseError) do
      CLIENT.audios(owner_id: 42_424_242_424_242_424_242_424_242)
    end
  end

  def test_user_with_locked_audios
    assert_raises(VkMusic::ParseError) do
      CLIENT.audios(owner_id: 152_719_703)
    end
  end

  def test_user_with_empty_audios
    pl = CLIENT.audios(owner_id: 437_727_675)
    assert_empty(pl, 'This user got no audios')
  end

  def test_group
    pl = CLIENT.audios(owner_id: -4_790_861)
    refute_empty(pl, 'There must be some music')
  end

  def test_group_with_upper_limit_1
    pl = CLIENT.audios(owner_id: -72_589_944, up_to: 10)
